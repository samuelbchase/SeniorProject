...
\caption{Creating a spanning tree on a graph with per-vertex degree constraints}
    ...
    \State T = $\emptyset$
        \While{there exits a vertex in $V$ with a degree $>$ 0 }
        \State {T+= ($v1$,$v2$) where $v1$ is the vertex of largest degree in $G$ and $v2$ is the vertex of smallest degree}
        \State {Decrement the degree of $v1$ and $v2$}
    \EndWhile
